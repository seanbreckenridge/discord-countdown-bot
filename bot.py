...
class ChannelBlacklist:
    ...
    def __init__(self, logger, multiplication_factor):
        # contains currently blacklisted channels and unix time at which they should be removed
        self.channels = {}
    ...
    def _remove_hanging_countdowns(self):
        ...
        for channel_id in list(self.channels):
            ...
            if self.channels[channel_id] < cur_time:
                self.logger.info(
                    f"Removed hanging countdown for channel id {channel_id}"
                )
    ...
    def start(self, channel_id: int, countdown_time: int):
        ...
        if channel_id not in self.channels:  # if there is no countdown
            self.channels[channel_id] = int(time.time()) + (
                countdown_time * self.multiplication_factor
            )
            self.logger.info(
                f"Started countdown in {channel_id} for {countdown_time} ({countdown_time * self.multiplication_factor} till next countdown)"
            )
        else:
...
COUNTDOWN_MIN = (
    3 if "COUNTDOWN_MIN" not in os.environ else int(os.environ["COUNTDOWN_MIN"])
)
COUNTDOWN_MAX = (
    15 if "COUNTDOWN_MAX" not in os.environ else int(os.environ["COUNTDOWN_MAX"])
)
...
short_blacklist: ChannelBlacklist = ChannelBlacklist(
    logger=log, multiplication_factor=2
)
...
def can_countdown(ctx):
    ...
    if server_mode == False:
        ...
        #
            ...
            return 'Unrecognized permissions. Ask an admin to run "{m} role" or "{m} bypass" to fix permissions.'.format(
                m=bot.user.mention
            )
...
async def start(ctx, *count_from):
    ...
    try:
        ...
        return await ctx.channel.send(
            "Could not convert {} to a number.".format(
                re.sub(
                    "[@`\s]",
                    "",
                    str(ve).replace("invalid literal for int() with base 10: ", "")[
                        1:-1
                    ],
                )
            )
    ...
    if parsed[0] > COUNTDOWN_MAX:
        return await ctx.channel.send(
            "{} is too damn high. {} is the maximum.".format(parsed[0], COUNTDOWN_MAX)
        )
    elif parsed[0] < COUNTDOWN_MIN:
        return await ctx.channel.send(
            "{} is too damn low. {} is the minimum.".format(parsed[0], COUNTDOWN_MIN)
        )
    if len(parsed) == 1:  # countdown starts now
        try:
            ...
            return await ctx.channel.send(
                "Can't start another countdown in this channel for {}s ".format(
                    time_left
                )
            )
...
async def count(num, ctx):
    ...
    send_at = time.time() + 1
    for i, countdown_message in zip(
        range(num, -1, -1), emoji_countdown_list(num, num_emoji)
    ):
...
async def time_cmd(ctx):
    ...
    if ctx.channel.id in last_run_at:
        ...
        return await ctx.channel.send(
            "Couldn't find when a countdown was last run in this channel..."
        )
...
async def role(ctx):
    ...
    if server_db.get(f"{ctx.guild.id}mode") == "!":
        await ctx.channel.send(
            "Removed bypass. The bot now requires a user to have the `#` role to use."
        )
    ...
    if "#" in [r.name for r in ctx.guild.roles]:
        ...
        if ctx.me.permissions_in(ctx.channel).manage_roles:
            ...
            return await ctx.channel.send(
                'Created the role `#` and gave it to you. Giving the `#` role to a user will allow them to start/stop countdowns. If you want everyone on the server to be able to use {m}, you can run "{m} bypass"'.format(
                    m=bot.user.mention
                )
            )
        else:
            return await ctx.channel.send(
                "Insufficient permissions to create the role. Create a role named `#`; any user with that role can start/stop countdowns."
            )
...
async def bypass(ctx):
    ...
    if server_db.get(f"{ctx.guild.id}mode") == "!":
        return await ctx.channel.send(
            f"Everyone on this server can already use {bot.user.mention}."
        )
    else:
        server_db.set(f"{ctx.guild.id}mode", "!")
        return await ctx.channel.send(
            f'Bypassed successfully. Anyone on the server can start/stop countdowns. If you wish to limit the use of the bot, run "{bot.user.mention} role"'
        )
...
async def status(ctx):
    ...
    if server_db.get(f"{ctx.guild.id}mode") == "!":
        return await ctx.channel.send(
            "The bot is currently set to 'bypass' mode. Anyone on the server can use the bot."
        )
    elif server_db.get(f"{ctx.guild.id}mode") == "#":
        return await ctx.channel.send(
            "The bot is currently set to 'role' mode. Anyone with the `#` role can use the bot."
        )
    else:
        return await ctx.channel.send(
            f'The bot hasn\'t been setup properly. Run "{bot.user.mention} help" for more information'
        )
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        ...
        try:
            ...
            return await ctx.channel.send(
                f'That command doesn\'t exist. Run "{bot.user.mention} help" for a list.'
            )
        ...
        log.info(
            "Invoking start from on_command_error with {} {}".format(
                message_parts, error
            )
        )
...
async def help(ctx):
    ...
    embed.add_field(
        name=f"@{ctx.guild.me.display_name} <n>",
        value="Start a countdown with length 'n'",
        inline=False,
    )
    embed.add_field(
        name=f"@{ctx.guild.me.display_name} stop",
        value="Stops the countdown in the current channel",
        inline=False,
    )
    embed.add_field(
        name=f"@{ctx.guild.me.display_name} time",
        value="Amount of time since a countdown was run in this channel",
        inline=False,
    )
    embed.add_field(
        name="Configuration Commands (requires admin)", value="\u200b", inline=False
    )
    embed.add_field(
        name=f"@{ctx.guild.me.display_name} status",
        value="Prints the current configuration status",
        inline=False,
    )
    embed.add_field(
        name=f"@{ctx.guild.me.display_name} role",
        value="Limits the bot usage to anyone with the `#` role.",
        inline=False,
    )
    embed.add_field(
        name=f"@{ctx.guild.me.display_name} bypass",
        value="Allows everyone to use the bot",
        inline=False,
    )
